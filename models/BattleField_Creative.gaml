...
global {
	...
	reflex nightTime when: night {
		ask BattleGrid {
			color <- #black;
		}
		ask Commander {
			if alliance != allianceLost {
				agentColor <- #peru;
			}
		}
		ask Soldier {
	...
	reflex restartBattle when: restartBattle and time - pauseTime = 2000 {
...
species Commander skills:[moving, fipa] {
    ...
    reflex fillAgentLists when: !initialLocationKnown and !night {
    	ask Medic {
    ...
    reflex doWander when: !night {
		do wander speed: 0.01;
	}
	
	reflex moveToTarget when: !night {
		do goto target:targetPoint speed: 0.3;
	}
	
	reflex knowInitialLocation when: !initialLocationKnown and !night {
		initialLocation <- location;
		initialLocationKnown <- true;
	}
	
	reflex sendInitiateBattle when: !battleInitiated and !night {
		if !cycleZero {
			cycleZero <- true;
			initialTime <- time;
		}
	...
	reflex receiveInforms when: !night {
	...
	reflex restoreProvisions when: !empty(provisionsMessageList) and !night {
	...
	reflex attackEnemy when: protectingZone and !night {
		ask Soldier {
	...
	reflex updateArmyHealth when: !night{
		if alliance = 1 {
...
species Soldier skills:[moving, fipa] {
	...
	reflex doWander when: !inField and !night {
		do wander speed: 0.01;
	}
	
	//Reflex to move reserve soldiers when an active one dies
	reflex moveToTargetReserve when: !inField and !night {
		do goto target:targetPoint speed: 0.2;
	}
	
	//Reflex to move active soldiers
	reflex moveToTarget when: battleIsHappening and inField and timeInFight = 0.0 and !wasFighting and !night{
		if (!injured) {
...
//					}
	...
	reflex moveToInitialPosition when: goingToProvisions or wasFighting and !night {
	...
	reflex stopMoving when: inField and !night {
		ask Soldier {
	...
	reflex moveInFight when: fighting and timeInFight != 0.0 and !night {
	...
	reflex goToProvisions when: bullets = 0 and !injured and !fighting and !night {
...
species Medic skills:[moving, fipa] {
	...
	reflex moveToTarget when: moveControl and !night {
...
species Transport skills:[moving, fipa] {
	...
	reflex injuredReported when: !night {
		loop element over: informs {
...
species Provisions skills: [fipa] {
    ...
    reflex restoreOwnProvisions when: medicineStock < 3 or bulletStock < 5 or fuelStock < 2 and !night {
		do start_conversation with: [ to :: list(Commander), protocol :: 'fipa-contract-net', 
